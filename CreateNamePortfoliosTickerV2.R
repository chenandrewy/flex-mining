...
create_single_set_returns_placebo <- function(seed = 1, n_tiles = 5 ){
  ...
  set.seed(seed)
  unique_names[!is.na(vector), signal := runif(.N)]
...
create_single_set_returns_edit <- function(word_to_compare, n_tiles = 5 ){
  ...
  unique_names[, signal := adist(lag_name, word_to_compare)]
...
create_names_portfolios_edit <- function(nports = 600,
                                    n_tiles = 5,
                                    word_list  = finance_words[, word_lower]){
  print(Sys.time())
  # nports = Number of portfolios
...
# %>%
  # filter(exchcd %in% c(1:3, 31:33))  %>% 
  # filter(shrcd %in% 10:11) %>% 
  # as.data.table()

# crspm_dt_processed[exchcd %in% c(1, 31) & !is.na(lag_me),
        # break_point_20 := quantile(lag_me, .2),
        # by = date]

# crspm_dt_processed[, break_point_20 := mean(break_point_20, na.rm = TRUE), by = date]

# crspm_dt_processed[, small_firms := lag_me < break_point_20]

# crspm_dt_processed[, small_firms := ifelse(is.na(small_firms), TRUE, small_firms)]

# crspm_dt_processed <- crspm_dt_processed[small_firms == FALSE, ]

unique_names <- crspm_dt_processed[, .(lag_name = unique(lag_name))]

##########################################

ticker_porfolios_dt <- create_names_portfolios_edit(6000, 5)

returns_dt <- crspm_dt_processed[!is.na(lag_me)
                                 & !is.na(ret),
                                 .(ew_mean = mean(ret, na.rm=TRUE),
                                   vw_mean = weighted.mean(ret, lag_me,
                                                           na.rm=TRUE)),
...
unique_names_2 <- copy(unique_names)
...
for (word_to_compare in freq_words$word[1:200]) {
  ...
  unique_names_2[, (word_to_compare) := f.rankit(sim_from_list(vector, sentence_to_vec(word_to_compare)))]
...
unique_3 <- unique_names_2[, -'vector']

unique_4 <- melt(unique_3, id.vars = 'lag_name', variable.name = 'word',
                 value.name = 'distance')

means_signal <- unique_4[!is.na(distance) & !is.na(lag_name), f.describe_numeric(distance), by = lag_name]
...
means_signal2 <- unique_4[!is.na(distance) & !is.na(lag_name), f.describe_numeric(distance), by = lag_name]


setkey(means_signal2, lag_name)
...
crspm_dt_processed[!is.na(bin1) & !is.na(lag_me)
...
crspm_dt_processed[!is.na(signal) & !is.na(lag_me)
                   & !is.na(ret), cor(cbind(signal, bin1, ret,
                                            lag_me),
                                      use ='pairwise.complete')]

crspm_dt_processed[, cor(signal, lag_me, 'pairwise.complete')]
...
crspm_dt_processed[!is.na(siccd), sic2 := str_extract(siccd, "^\\d{2}")]

a <- crspm_dt_processed[!is.na(signal) & !is.na(sic2), .(signal = mean(signal),
                                                          bin1 = sum(bin1 == 1),
                                                          bin5 = sum(bin1 == 5),
...
b <- crspm_dt_processed[, .N, by = .(bin, yyyymm)]
...
for (word_to_compare in finance_words[1:60, word_lower]) {
  ...
  crspm_dt_processed[unique_names, (word_to_compare) := signal]
  
  crspm_dt_processed[!is.na(get(word_to_compare)), (names_bin) := fntile(get(word_to_compare), 5), by = yyyymm]
