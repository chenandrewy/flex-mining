...
matchsum[, abs_t := abs(tstat)]
matchsum[, abs_t_ge_2 := abs_t >= 2]
matchsum[, sign := sign(rbar)]
# Make matched panel ------------------------------------------------------
...
saveRDS(matchdat,
        file = paste0('../Data/Processed/', DMshortname, ' MatchPubTG2.RData')
)
...
for (act in unique(candidateReturns$actSignal)) {
  print(act)
  tmp = candidateReturns %>% 
...
saveRDS(allRhos, '../Results/PairwiseCorrelationsActualAndMatchesTG2.RDS')
