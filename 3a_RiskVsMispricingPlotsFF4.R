...
# FF4
reg_ff4 = lm(ret ~ risk_via_ff4, average_by_ntile) %>%
  summary()

p2 <- ggplot(average_by_ntile, aes(x=risk_via_ff4, y=ret)) +
  geom_hline(yintercept = 0, color = 'gray', size = 2) +
  geom_vline(xintercept =  0, color = 'gray', size = 1) +
  geom_hline(yintercept = 1, color = 'gray', size = 2) +
  geom_point(size = 2.5) +
  ...
  xlim(xax) + ylim(c(-1, 2))    +
  geom_abline(
    aes(intercept = reg_ff4$coefficients[1], slope = reg_ff4$coefficients[2])
    , color = colors[1], size = 2
  )

p2

ggsave('../Results/Fig_Risk_via_FF4.pdf', p2, width = 10, height = 8)
