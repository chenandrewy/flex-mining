...
printme = ReturnPlotsWithDM4series(
  ...
  legendlabels =
    c(
      paste0("Published"),
      paste0("Adj. R2 > 0.25 using 5 PCs of available pubs"),
      paste0("Adj. R2 < 0.25 using 5 PCs of available pubs\n and t-stat > t pub"),
      paste0("Adj. R2 < 0.25 using 5 PCs of available pubs\n and t-stat <= t pub")
    ),
  legendpos = c(30,30)/100,
...
printme = ReturnPlotsWithDM4series(
  ...
  legendlabels =
    c(
      paste0("Published"),
      paste0("Cor > 0.5 w/ all current or previous pubs"),
...
tab_span2_melt$variable <- factor(tab_span2_melt$variable, 
                                  ...
                                  labels = c("Cummulative # of DM Portfolios",
                                             "# of Spanned DM Portfolios",
                                             "# of Unspanned DM Portfolios"))
...
ggsave("../Results/Fig_spanned_over_time.pdf", plot = plot, device = "pdf", width = 10, height = 6)
...
# Save the plot to a PDF file
ggsave("../Results/Fig_spanned_over_time_pca.pdf", plot = plot, device = "pdf", width = 10, height = 6)
...
# Prepare the table
table_latex_to_latex <- name_cluster %>%
  group_by(cluster) %>%
  summarize(nstrat = n()) %>%
  left_join(
    name_cluster %>%
      group_by(cluster) %>%
      arrange(cluster, -abs(mean_tstat)) %>%
      slice(1),
    by = 'cluster'
  ) %>%
  transmute(
    `Cluster` = cluster,
    `N` = nstrat,
    `Example Portfolio` = desc,
    `Example t-stat` = mean_tstat
  )

# Convert to LaTeX table
table_latex <- xtable(table_latex_to_latex, caption = "Spanned Clusters", label = "tab:clusters_spanned")

# Set the table alignment and format
align(table_latex) <- "llllc"
digits(table_latex) <- c(0, 0, 0, 0, 1)  # Adjust the number of digits for each column

# Print the LaTeX code
print(table_latex, type = "latex", include.rownames = FALSE, caption.placement = "top", 
      hline.after = seq(from = 0, to = nrow(table_latex_to_latex), by = 1))
...
# Convert to LaTeX table
table_latex <- xtable(table_latex_to_latex, caption = "Unspanned Clusters", label = "tab:clusters")
...
# pull list of spanned dm strats
dmcodelist = dt_with_spanned_ever %>% 
  .[floor(sampend) == ycur & spanned_ever == TRUE
    , .(sweight, dmname)] %>% 
  distinct(sweight, dmname) %>% 
  merge(dm_linktable, by = c('sweight', 'dmname')) %>% 
...
name_cluster = data.table(dmcode = names(sub_grp), cluster = sub_grp) %>% 
  left_join(dm_linktable[,.(dmcode, shortdesc, desc)], by='dmcode') %>%
  left_join(
    dt_with_spanned_ever[floor(sampend) == ycur]  %>% 
      mutate(dmcode = paste0(sweight, '|', dmname)) %>% 
      group_by(dmcode) %>% summarize(mean_tstat = mean((tstat))) %>% ungroup() %>% 
      select(dmcode, mean_tstat)
    , by='dmcode') %>% 
...
# Convert to LaTeX table
table_latex <- xtable(table_latex_to_latex, caption = "Spanned Clusters - PCA", label = "tab:clusters_spanned")
...
# Convert to LaTeX table
table_latex <- xtable(table_latex_to_latex, caption = "Unspanned Clusters - PCA", label = "tab:clusters")
...
print(table_latex, type = "latex", include.rownames = FALSE, caption.placement = "top", 
      hline.after = seq(from = 0, to = nrow(table_latex_to_latex), by = 1))
# One table like current (t-stats quantiles )
# 2nd table 5 examples 