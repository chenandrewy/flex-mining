...
unique_tickers <- crspm_dt_finance_words[, .(lag_ticker = unique(lag_ticker))]
...
setkey(crspm_dt_finance_words, lag_ticker)

dt_save  <- data.table()

for (word_to_compare in finance_words[1:60, word_lower]) {
  
  # unique_tickers[, signal := adist(lag_ticker, word_to_compare)/pmax(nchar(lag_ticker), nchar(word_to_compare))]
  
  # unique_tickers[, signal_j := stringdist(word_to_compare, lag_ticker, method = 'jaccard')]
  
  unique_tickers[, signal_c := stringdist(word_to_compare, lag_ticker, method = 'cosine')]
  
  crspm_dt_finance_words[unique_tickers, signal := signal_c]
  
  crspm_dt_finance_words[, word := word_to_compare]
  
  unique_tickers[, signal_c := NULL]

  dt_save_loop  <- crspm_dt_finance_words[, .(permno, yyyymm, signal, word)]
  
  crspm_dt_finance_words[, signal := NULL]
  
  crspm_dt_finance_words[, word := NULL]
  
  dt_save <- rbind(dt_save, dt_save_loop)
