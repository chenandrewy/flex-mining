# Full Sample Risk-Adjusted Data-mining comparisons (FF4)
# Based on 4c4_RiskAdjustedResearchVsDMPlotsTV.R but with full sample CAPM and FF4 adjustments
...
## Full sample CAPM/FF4 adjustments
czret %>% setDT()
...
# Compute published-side full sample CAPM/FF4 betas and normalized alphas
...
ff4_fs <- czret[date >= sampstart & !is.na(ret) & !is.na(mktrf) & !is.na(smb) & !is.na(hml) & !is.na(umd), {
  coeffs <- extract_ff4_coeffs(ret, mktrf, smb, hml, umd)
  .(beta_ff4_fs = coeffs[1], s_ff4_fs = coeffs[2], h_ff4_fs = coeffs[3], u_ff4_fs = coeffs[4])
}, by = signalname]
czret <- merge(czret, ff4_fs, by = 'signalname', all.x = TRUE)
czret[, abnormal_ff4 := ret - (beta_ff4_fs * mktrf + s_ff4_fs * smb + h_ff4_fs * hml + u_ff4_fs * umd)]
# FIXED: Use sampstart/sampend periods for computing in-sample statistics
czret[date >= sampstart & date <= sampend, `:=`(
  abar_ff4_fs = mean(abnormal_ff4, na.rm = TRUE),
  abar_ff4_fs_t = {
    m <- mean(abnormal_ff4, na.rm = TRUE)
    s <- sd(abnormal_ff4, na.rm = TRUE)
    n <- sum(!is.na(abnormal_ff4))
    if (n > 1 && s > 0) m / s * sqrt(n) else NA_real_
  }
), by = signalname]
czret[, abar_ff4_fs := nafill(abar_ff4_fs, 'locf'), by = .(signalname)]
czret[, abar_ff4_fs_t := nafill(abar_ff4_fs_t, 'locf'), by = .(signalname)]
czret[, abnormal_ff4_fs_normalized := ifelse(abs(abar_ff4_fs) > 1e-10, 100 * abnormal_ff4 / abar_ff4_fs, NA_real_)]
...
ret_for_plot0_adj <- ret_for_plot0 %>%
  left_join(
    czret %>% select(signalname, eventDate,
                      abnormal_capm, abnormal_ff4,
                      abnormal_capm_fs_normalized, abnormal_ff4_fs_normalized),
    by = c("pubname" = "signalname", "eventDate" = "eventDate")
  )
...
print(paste("Number of signals with full sample FF4 adjustments:", sum(!is.na(czret$beta_ff4_fs))))
...
if("abnormal_capm" %in% names(candidateReturns_adj) && "abnormal_ff4" %in% names(candidateReturns_adj)) {
  ...
  dm_stats_fs <- candidateReturns_adj[
    ...
    .(
      ...
      abar_ff4_fs_dm = mean(abnormal_ff4, na.rm = TRUE)
    ),
  ...
  signals_ff4_fs_t2  <- unique(czret[abar_ff4_fs_t  > t_threshold]$signalname)
  ...
  signals_ff4_fs_t2  <- intersect(signals_ff4_fs_t2, signals_raw_t2)
  ...
  # FF4 full sample filtering (t-stat only)
  cat("\n=== FF4 FULL SAMPLE (t > ", t_threshold, ") STATISTICS ===\n")
  dm_filtered_ff4_fs <- candidateReturns_adj %>%
    inner_join(
      dm_stats_fs %>% filter(abar_ff4_fs_dm_t > t_threshold),
      by = c("actSignal", "candSignalname")
    )

  # Normalize and aggregate
  dm_ff4_fs_aggregated <- normalize_and_aggregate_dm_fs(
    dm_filtered_ff4_fs,
    "abnormal_ff4",
    "ff4_fs_t2_normalized"
  )

  # Create filtered plot data
  ret_for_plot0_ff4_fs_t2 <- create_filtered_plot_data_fs(
    ret_for_plot0_adj,
    signals_ff4_fs_t2,  # Use FS-filtered published signals
    dm_ff4_fs_aggregated,
    "abnormal_ff4_fs_normalized",  # Use published FS-normalized alpha
    "matchRet_ff4_fs_t2_normalized",
    "ff4_fs_t2_normalized"
  )

  cat("Published signals with FF4 t > ", t_threshold, ":", length(signals_ff4_fs_t2), "\n")
  cat("Published signals with filtered DM matches (full sample):", length(unique(ret_for_plot0_ff4_fs_t2$pubname)), "\n")
  cat("DM signals with full sample FF4 t > ", t_threshold, ":", sum(dm_stats_fs$abar_ff4_fs_dm_t > t_threshold, na.rm = TRUE), "\n")

  # Create and save plot
  printme_ff4_fs_t2 <- create_full_sample_risk_adjusted_plot(
    ret_for_plot0_ff4_fs_t2,
    "abnormal_ff4_fs_normalized",
    "matchRet_ff4_fs_t2_normalized",
    "FF4 Alpha",
    t_threshold,
  ...
  fs_plot_data[["ff4_fs"]] <- ret_for_plot0_ff4_fs_t2
  ...
  print_summary_table_fs(
    ...
    analysis_types = c("raw", "capm_fs", "ff4_fs"),
...
if("abnormal_capm" %in% names(candidateReturns_adj) && exists("ret_for_plot0_capm_fs_t2")) {
  ...
  # FF4 full sample t > t_threshold filtered (by theory)
  ff4_fs_t2_summary_theory <- compute_outperformance_fs(
    ret_for_plot0_ff4_fs_t2,
    "abnormal_ff4_fs_normalized", "matchRet_ff4_fs_t2_normalized", theory_mapping, "theory_group"
  )
  ...
  # FF4 full sample t > t_threshold filtered (by model)
  ff4_fs_t2_summary_model <- compute_outperformance_fs(
    ret_for_plot0_ff4_fs_t2,
    "abnormal_ff4_fs_normalized", "matchRet_ff4_fs_t2_normalized", model_mapping, "modeltype_grouped"
  )
...
overall_t2_summary_ff4_fs <- compute_overall_summary_fs(
  plot_data = ret_for_plot0_ff4_fs_t2,
  ret_col = "abnormal_ff4_fs_normalized",
  dm_col = "matchRet_ff4_fs_t2_normalized"
)
...
# FF4 full sample t > t_threshold filtered (by discipline)
ff4_fs_t2_summary_discipline <- compute_outperformance_fs(
  ret_for_plot0_ff4_fs_t2,
  "abnormal_ff4_fs_normalized", "matchRet_ff4_fs_t2_normalized", discipline_mapping_filtered, "discipline"
)
...
# FF4 full sample t > t_threshold filtered (by journal)
ff4_fs_t2_summary_journal <- compute_outperformance_fs(
  ret_for_plot0_ff4_fs_t2,
  "abnormal_ff4_fs_normalized", "matchRet_ff4_fs_t2_normalized", journal_mapping_filtered, "journal_rank"
)
...
# FF4 full sample t > t_threshold filtered (by model binary)
ff4_fs_t2_summary_anymodel <- compute_outperformance_fs(
  ret_for_plot0_ff4_fs_t2,
  "abnormal_ff4_fs_normalized", "matchRet_ff4_fs_t2_normalized", anymodel_mapping, "model_binary"
)
}
...
for(group in groups_theory) {
  ...
  ff4_fs_ret <- round(get_values(ff4_fs_t2_summary_theory, "theory_group", group, "pub_oos"))

  # Outperformance
  capm_fs_out <- round(get_values(capm_fs_t2_summary_theory, "theory_group", group, "outperform"))
  ff4_fs_out <- round(get_values(ff4_fs_t2_summary_theory, "theory_group", group, "outperform"))

  # Standard errors
  capm_fs_se <- round(get_values(capm_fs_t2_summary_theory, "theory_group", group, "pub_oos_se"))
  ff4_fs_se <- round(get_values(ff4_fs_t2_summary_theory, "theory_group", group, "pub_oos_se"))

  capm_fs_out_se <- round(get_values(capm_fs_t2_summary_theory, "theory_group", group, "outperform_se"))
  ff4_fs_out_se <- round(get_values(ff4_fs_t2_summary_theory, "theory_group", group, "outperform_se"))

  cat(sprintf("%-12s           %4s     %4s          %4s     %4s\n",
              group, capm_fs_ret, ff4_fs_ret, capm_fs_out, ff4_fs_out))
  cat(sprintf("%-12s           (%2s)    (%2s)          (%2s)    (%2s)\n",
              "", capm_fs_se, ff4_fs_se, capm_fs_out_se, ff4_fs_out_se))
}
...
for(group in groups_model) {
  ...
  ff4_fs_ret <- round(get_values(ff4_fs_t2_summary_model, "modeltype_grouped", group, "pub_oos"))

  # Outperformance
  capm_fs_out <- round(get_values(capm_fs_t2_summary_model, "modeltype_grouped", group, "outperform"))
  ff4_fs_out <- round(get_values(ff4_fs_t2_summary_model, "modeltype_grouped", group, "outperform"))

  # Standard errors
  capm_fs_se <- round(get_values(capm_fs_t2_summary_model, "modeltype_grouped", group, "pub_oos_se"))
  ff4_fs_se <- round(get_values(ff4_fs_t2_summary_model, "modeltype_grouped", group, "pub_oos_se"))

  capm_fs_out_se <- round(get_values(capm_fs_t2_summary_model, "modeltype_grouped", group, "outperform_se"))
  ff4_fs_out_se <- round(get_values(ff4_fs_t2_summary_model, "modeltype_grouped", group, "outperform_se"))
...
# Overall
cat(sprintf("%-12s           %4s     %4s          %4s     %4s\n",
            "All", round(overall_t2_summary_capm_fs$pub_oos), round(overall_t2_summary_ff4_fs$pub_oos),
            round(overall_t2_summary_capm_fs$outperform), round(overall_t2_summary_ff4_fs$outperform)))
cat(sprintf("%-12s           (%2s)    (%2s)          (%2s)    (%2s)\n",
            "", round(overall_t2_summary_capm_fs$pub_oos_se), round(overall_t2_summary_ff4_fs$pub_oos_se),
            round(overall_t2_summary_capm_fs$outperform_se), round(overall_t2_summary_ff4_fs$outperform_se)))
...
for(i in 1:length(groups_theory)) {
  ...
  fs_theory_data[[i]] <- list(
    ...
    # FF4
    ff4_fs_pub_oos = get_values(ff4_fs_t2_summary_theory, "theory_group", group, "pub_oos"),
    ff4_fs_pub_oos_se = get_values(ff4_fs_t2_summary_theory, "theory_group", group, "pub_oos_se"),
    ff4_fs_outperform = get_values(ff4_fs_t2_summary_theory, "theory_group", group, "outperform"),
    ff4_fs_outperform_se = get_values(ff4_fs_t2_summary_theory, "theory_group", group, "outperform_se")
  )
}
...
for(i in 1:length(groups_model)) {
  ...
  fs_model_data[[i]] <- list(
    ...
    # FF4
    ff4_fs_pub_oos = get_values(ff4_fs_t2_summary_model, "modeltype_grouped", group, "pub_oos"),
    ff4_fs_pub_oos_se = get_values(ff4_fs_t2_summary_model, "modeltype_grouped", group, "pub_oos_se"),
    ff4_fs_outperform = get_values(ff4_fs_t2_summary_model, "modeltype_grouped", group, "outperform"),
    ff4_fs_outperform_se = get_values(ff4_fs_t2_summary_model, "modeltype_grouped", group, "outperform_se")
  )
}
...
fs_overall_data <- list(
  ...
  # FF4
  ff4_fs_pub_oos = overall_t2_summary_ff4_fs$pub_oos,
  ff4_fs_pub_oos_se = overall_t2_summary_ff4_fs$pub_oos_se,
  ff4_fs_outperform = overall_t2_summary_ff4_fs$outperform,
  ff4_fs_outperform_se = overall_t2_summary_ff4_fs$outperform_se
)
...
for(i in 1:length(discipline_groups)) {
  ...
  fs_discipline_data[[i]] <- list(
    ...
    # FF4
    ff4_fs_pub_oos = get_values(ff4_fs_t2_summary_discipline, "discipline", group, "pub_oos"),
    ff4_fs_pub_oos_se = get_values(ff4_fs_t2_summary_discipline, "discipline", group, "pub_oos_se"),
    ff4_fs_outperform = get_values(ff4_fs_t2_summary_discipline, "discipline", group, "outperform"),
    ff4_fs_outperform_se = get_values(ff4_fs_t2_summary_discipline, "discipline", group, "outperform_se")
  )
}
...
for(i in 1:length(journal_groups)) {
  ...
  fs_journal_data[[i]] <- list(
    ...
    # FF4
    ff4_fs_pub_oos = get_values(ff4_fs_t2_summary_journal, "journal_rank", group, "pub_oos"),
    ff4_fs_pub_oos_se = get_values(ff4_fs_t2_summary_journal, "journal_rank", group, "pub_oos_se"),
    ff4_fs_outperform = get_values(ff4_fs_t2_summary_journal, "journal_rank", group, "outperform"),
    ff4_fs_outperform_se = get_values(ff4_fs_t2_summary_journal, "journal_rank", group, "outperform_se")
  )
}
...
fs_anymodel_data <- list(
  ...
  # FF4
  ff4_fs_pub_oos = get_values(ff4_fs_t2_summary_anymodel, "model_binary", "No Model", "pub_oos"),
  ff4_fs_pub_oos_se = get_values(ff4_fs_t2_summary_anymodel, "model_binary", "No Model", "pub_oos_se"),
  ff4_fs_outperform = get_values(ff4_fs_t2_summary_anymodel, "model_binary", "No Model", "outperform"),
  ff4_fs_outperform_se = get_values(ff4_fs_t2_summary_anymodel, "model_binary", "No Model", "outperform_se")
)
...
# Export full sample tables if they exist (Raw + CAPM + FF4)
if (exists("fs_theory_data") && exists("fs_model_data")) {
  ...
  if (exists("fs_anymodel_data")) {
    ...
    fs_anymodel_any <- list(
      ...
      # FF4
      ff4_fs_pub_oos = mean(c(
        get_values(ff4_fs_t2_summary_model, "modeltype_grouped", "Stylized", "pub_oos"),
        get_values(ff4_fs_t2_summary_model, "modeltype_grouped", "Dynamic or Quantitative", "pub_oos")
      ), na.rm = TRUE),
      ff4_fs_pub_oos_se = sqrt(mean(c(
        get_values(ff4_fs_t2_summary_model, "modeltype_grouped", "Stylized", "pub_oos_se")^2,
        get_values(ff4_fs_t2_summary_model, "modeltype_grouped", "Dynamic or Quantitative", "pub_oos_se")^2
      ), na.rm = TRUE)),
      ff4_fs_outperform = mean(c(
        get_values(ff4_fs_t2_summary_model, "modeltype_grouped", "Stylized", "outperform"),
        get_values(ff4_fs_t2_summary_model, "modeltype_grouped", "Dynamic or Quantitative", "outperform")
      ), na.rm = TRUE),
      ff4_fs_outperform_se = sqrt(mean(c(
        get_values(ff4_fs_t2_summary_model, "modeltype_grouped", "Stylized", "outperform_se")^2,
        get_values(ff4_fs_t2_summary_model, "modeltype_grouped", "Dynamic or Quantitative", "outperform_se")^2
      ), na.rm = TRUE))
    )
