...
comnams_dt <- crspm_dt[!is.na(comnam), .(comnam = unique(comnam))]

counts_df <- comnams_dt[!is.na(comnam), ] %>%
...
text_df <- comnams_dt[!is.na(comnam), ] %>%
  unnest_tokens(word, comnam)%>%
  anti_join(stop_words, by= c("word" = "word"))%>%
  filter(nchar(word) > 2) %>%
  group_by(I) %>%
  summarise(processed_conm = paste0(word, collapse = ' ')) %>%
  select(processed_conm)  %>%
  as.data.table()
...
for (word_to_compare in word_list_finance) {
  
  text_df[,
             (word_to_compare) :=
               adist(processed_conm, word_to_compare)/pmax(len_name, nchar(word_to_compare)) ]
  
  # print(word_to_compare)
}

cors_measure <- text_df[, cor(.SD), .SDcols = word_list_finance[1:10]]
#############################################
# Month format
...
crspm_dt[ ,
          comnam :=  zoo::na.locf(comnam,na.rm =  FALSE),
          by = permno]


# Temporary comnam so that new stocks are added in June
crspm_dt[month(date) == 5, tic_temp := tolower(comnam)]

# comnam available in real time
crspm_dt[, lag_comnam := shift(tic_temp, 1),
         by = .(permno)]

# Fill-forward missing comnams
crspm_dt[ ,
          lag_comnam :=  zoo::na.locf(lag_comnam,na.rm =  FALSE),
          by = permno]

# Lagged market cap
crspm_dt[, lag_me := shift(me), by = permno]

# Remove if missing lagged comnam or lagged market cap

crspm_dt <- crspm_dt[!is.na(lag_me) & !is.na(lag_comnam) & nchar(lag_comnam) >= 3, ]

ns <- crspm_dt[, .N, by = yyyymm]

crspm_dt_finance_words <- crspm_dt[, .(permno, yyyymm, lag_comnam)]

comnams_dt <- crspm_dt_finance_words[, .(lag_comnam = unique(lag_comnam))]
...
setkey(comnams_dt, lag_comnam)



write_fst(comnam_porfolios_jane_dt,
